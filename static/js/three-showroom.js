 ...
 * 360° Drag-to-Rotate WebGL viewer with OrbitControls and 4 interactive pulsing hotspots.
 */
...
(function() {
  ...
  const violetKey = new THREE.PointLight(0x8B5CF6, 4.0, 18);
  violetKey.position.set(-4, 3, 2);
  scene.add(violetKey);

  const cyanFill = new THREE.PointLight(0x06B6D4, 3.5, 18);
  cyanFill.position.set(4, -2, 2);
  scene.add(cyanFill);
  ...
  // 5. 3D Hotspot Coordinate Tracking
  const hotspotData = [
    {
      id: 'hotspotCamera',
    ...
    {
      id: 'hotspotDisplay',
      localPos: new THREE.Vector3(0, 0.5, 0.15),
      title: 'Quantum OLED 120Hz',
      desc: 'ProMotion LTPO display with 3000 nits peak brightness and HDR10+ certification.'
    },
    {
      id: 'hotspotChip',
      localPos: new THREE.Vector3(0, -0.6, 0.15),
      title: '3nm Neural Silicon',
      desc: 'Nexus Bionic Core delivering 35 Trillion operations/sec on-device AI computing.'
    },
    {
      id: 'hotspotBattery',
      localPos: new THREE.Vector3(0, -1.8, 0.15),
      title: '5000mAh Ultra-Dense Battery',
      desc: '65W Turbo GaN charging engineered for up to 36 hours of continuous creative work.'
    }
  ];
  ...
  const updateHotspotPositions = () => {
    ...
    hotspotData.forEach(h => {
      ...
      // Check if point is facing the camera or behind the object
  ...
  const animate = () => {
    ...
    device.position.y = Math.sin(elapsed * 1.5) * 0.1;
